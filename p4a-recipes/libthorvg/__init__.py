...
class LibThorVGRecipe(MesonRecipe):
    ...
    def _find_clang_libomp(self, arch):
        llvm_dir = self.ctx.ndk.llvm_prebuilt_dir
        arch_map = {
            "arm64-v8a": "aarch64",
            "armeabi-v7a": "arm",
            "x86": "i386",
        ...
        for lib_subdir in ("lib", "lib64"):
            pattern = join(llvm_dir, lib_subdir, "clang", "*", "lib", "linux", lib_arch)
            matches = glob(pattern)
            if matches:
                return join(matches[0], "libomp.so")

        raise FileNotFoundError(
            f"Cannot find libomp.so for {arch.arch} in NDK at {llvm_dir}"
        )
    ...
    def build_arch(self, arch):
        ...
        with current_directory(build_dir):
            ...
            libomp = self._find_clang_libomp(arch)
